...
if Player_Monitor == True:
  obstacle_spawn_time = 8000 # Spawnt Objekte am Anfang alle 8 Sekunden
else:
...
async def main():
    ...
    while run:
        ...
        if pygame.time.get_ticks() - last_obstacle_spawn_time > obstacle_spawn_time:
            ...
            if Player_Monitor == True:
              obstacle_speed = obstacle_speed + screen_height * 0.00001667 * 5 * 1.5 # Objekte werden nach dem spawnen schneller
            else:
              obstacle_speed = obstacle_speed + screen_height * 0.00001667 * 1.5 # Objekte werden nach dem spawnen schneller
            last_obstacle_spawn_time = pygame.time.get_ticks()
            
            if Player_Monitor == True:
              if obstacle_spawn_time < 1500: # Begrenzt die Spawngeschwindigkeit bei max 1,5 Sekunden
